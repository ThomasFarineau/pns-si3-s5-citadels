...
class ExpGoldStrategyTest {
    ...
    void setUp() {
        ...
        player = new Player("ExpGoldStrategy", ExpGoldStrategy.class);
    }

    @Test
    void chooseGoldOrDistrict_GOLD() {
        District districtTemple = new District(DistrictType.TEMPLE);
        player.getDistrictsHand().add(districtTemple);
        ...
        // Le joueur a un quartier alors il va choisir l'or
    }
    ...
    void chooseGoldOrDistrict_DISTRICT() {
        ...
        // Le joueur n'a pas de quartier alors il va choisir de piocher
    }
    ...
    void chooseDistrict() {
        ...
        assertEquals(districtTemple, player.chooseDistrict(districtTemple, districtCathedral)[0]);
        assertTrue(player.getDistrictsHand().contains(districtCathedral));
        // Le joueur va toujours choisir le quartier le plus cher donc la méthode renvoie le quartier qu'elle ne choisit pas
    }
    ...
    void chooseDistrict2() {
        ...
        assertTrue(player.getDistrictsHand().contains(districtCathedral));
        // Le joueur va toujours choisir le quartier le plus cher
    }
    ...
    void placeDistrict() {
        ...
        assertTrue(player.getDistrictsBoard().contains(districtCathedral));
        //Le joueur va toujours placé le quartier le plus chere
    }
    ...
    void placeDistrict2() {
        ...
        // S'il n'a pas les moyens de prendre le quartier le plus chere, alors il ne fait rien
    }

    @Test
    void whatToDoTestPlaceDistrict() {
        player.chooseCharacter(new Character(CharacterType.KING));
        player.getDistrictsHand().addDistricts(new District(DistrictType.OBSERVATORY));
        player.giveGold(10);
